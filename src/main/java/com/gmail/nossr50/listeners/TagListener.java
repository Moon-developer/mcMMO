...
public class TagListener implements Listener {
    ...
    public void onAsyncPlayerReceiveNameTag(AsyncPlayerReceiveNameTagEvent event) {
        ...
        if (ChatColor.stripColor(tag).equals(tag)) {
            mcMMOPlayer.setNameTag(null);
            return;
        }

        if (ChatColor.stripColor(tag).equals(player.getName()) && (mcMMOPlayer.getNameTag() == null || !mcMMOPlayer.getNameTag().equalsIgnoreCase(tag))) {
